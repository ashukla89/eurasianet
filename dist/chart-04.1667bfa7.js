...
var svg = d3.select('#chart-04').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // Create your scales

var xPositionScale = d3.scaleLinear().range([0, width]);
var yPositionScale = d3.scaleLinear().range([height, 0]); // const colorScale = d3.scaleOrdinal().range(['#1b9e77', '#d95f02', '#7570b3'])
...
d3.csv(require('../data/air-emissions.csv')).then(ready).catch(function (err) {
  console.log(err);
});
...
function ready(datapoints) {
  ...
  var minYear = d3.min(datapoints, function (d) {
    return d.Year;
  });
  var maxYear = d3.max(datapoints, function (d) {
    return d.Year;
  });
  xPositionScale.domain([minYear, maxYear]);
  yPositionScale.domain([0, 500]); // Draw your dots
  ...
  svg.selectAll('circle').data(nested).enter().append('circle').attr('cx', width).attr('cy', function (d) {
    ...
    var lastYear = datapoints[datapoints.length - 1]; // '+' forces it to numeric; is read in as string
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('fill', 'none').attr('stroke-width', 1).attr('stroke', function (d) {
    if (d.key === 'France') {
      return 'blue';
    } else {
      return 'grey';
  ...
  svg.selectAll('text').data(nested).enter().append('text').text(function (d) {
    ...
    var lastYear = datapoints[datapoints.length - 1]; // Use the yPositionScale and the high temperature
    // of December to position our text

    return yPositionScale(lastYear.Value);
  }).attr('font-size', 12).attr('dx', 5) // offset of 5 pixels to the right
  // .attr('dy', function(d) {
  //   // nothing needed here
  // })
  .attr('alignment-baseline', 'middle').attr('fill', function (d) {
    if (d.key === 'France') {
      return 'blue';
    } else {
      return 'grey';
...
},{"d3":"../node_modules/d3/index.js","../data/air-emissions.csv":"data/air-emissions.csv"}]},{},["scripts/chart-04.js"], null)
//# sourceMappingURL=/chart-04.1667bfa7.js.map