...
var width = 500 - margin.left - margin.right; // Add your svg

console.log('Building chart 5');
var svg = d3.select('#chart-05').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // Create a time parser (see hints)

var parseTime = d3.timeParse('%B-%y'); // evaluate
// console.log(parseTime('August-17'))
// Create your scales

var xPositionScale = d3.scaleLinear().range([0, width]);
var yPositionScale = d3.scaleLinear().range([height, 0]);
var colorScale = d3.scaleOrdinal().range(['#8dd3c7', '#ffffb3', '#bebada', '#fb8072', '#80b1d3', '#fdb462', '#b3de69', '#fccde5', '#d9d9d9', '#bc80bd']); // Create a d3.line function that uses your scales
...
d3.csv(require('../data/housing-prices.csv')).then(ready).catch(function (err) {
  console.log(err);
...
function ready(datapoints) {
  ...
  var prices = datapoints.map(function (d) {
    return d.price;
  });
  xPositionScale.domain(d3.extent(dates));
  yPositionScale.domain(d3.extent(prices)); // Group your data together

  var nested = d3.nest().key(function (d) {
    return d.region;
  }).entries(datapoints); // Draw your lines

  svg.selectAll('path').data(nested).enter().append('path').attr('fill', 'none').attr('stroke-width', 2).attr('stroke', function (d) {
    return colorScale(d.key);
  }).attr('d', function (d) {
    return line(d.values);
  }).attr('opacity', 1); // Add your text on the right-hand side

  svg.selectAll('text').data(nested).enter().append('text').text(function (d) {
    return d.key;
  }).attr('x', width).attr('y', function (d) {
    var datapoints = d.values; // Find the last datapoint

    var lastMonth = datapoints[0]; // Use the yPositionScale and the high temperature
    // of December to position our text

    return yPositionScale(lastMonth.price);
  }).attr('font-size', 10).attr('dx', 5) // offset of 5 pixels to the right
  // .attr('dy', function(d) {
  //   // nothing needed here
  // })
  .attr('alignment-baseline', 'middle').attr('fill', 'black'); // Draw your dots

  svg.selectAll('circle').data(nested).enter().append('circle').attr('cx', width).attr('cy', function (d) {
    var datapoints = d.values; // Find the last datapoint
  ...
  svg.append('text').text('U.S. housing prices fall in winter').attr('x', width / 2) // in the center
  .attr('text-anchor', 'middle') // center aligned
  .attr('dy', -30).attr('font-size', 20); // Add the shaded rectangle

  svg.append('rect').attr('x', function (d) {
    return xPositionScale(parseTime('December-16'));
  }).attr('y', 0).attr('width', function (d) {
    return xPositionScale(parseTime('February-17')) - xPositionScale(parseTime('December-16'));
  }).attr('height', height).attr('fill', 'lightgrey').lower(); // Add your axes
  ...
  var yAxis = d3.axisLeft(yPositionScale).ticks(7);
  svg.append('g').attr('class', 'axis y-axis').call(yAxis);
}
},{"d3":"../node_modules/d3/index.js","../data/housing-prices.csv":"data/housing-prices.csv"}]},{},["scripts/chart-05.js"], null)
