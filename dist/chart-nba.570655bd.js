...
var width = 450 - margin.left - margin.right; // Add your svg

var svg = d3.select('#chart-nba').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // Create your scales

var xPositionScale = d3.scaleLinear().range([0, width]);
var yPositionScale = d3.scaleLinear().range([height, 0]);
var radiusScale = d3.scaleSqrt().range([0, 7]);
var colorScale = d3.scaleOrdinal().range(['#7fc97f', '#beaed4', '#fdc086', '#ffff99']); // Read in your housing price data

d3.csv(require('../data/player_stats.csv')).then(ready).catch(function (err) {
  console.log(err);
}); // Write your ready function

function ready(datapoints) {
  // Define domains for scales
  var minPts = d3.min(datapoints, function (d) {
    return d.PPG;
  });
  var maxPts = d3.max(datapoints, function (d) {
    return d.PPG;
  });
  xPositionScale.domain([170, 230]);
  yPositionScale.domain([60, 160]);
  radiusScale.domain([minPts, maxPts]); // Draw your dots

  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('cx', function (d) {
    return xPositionScale(d.Height);
  }).attr('cy', function (d) {
    return yPositionScale(d.Weight);
  }).attr('r', function (d) {
    return radiusScale(d.PPG);
  }).attr('fill', function (d) {
    return colorScale(d.Pos);
  }).attr('opacity', 0.5).attr('class', function (d) {
    return d.Pos;
  }).on('mouseover', function (d, i) {
    console.log('mouseover on', this);
    d3.select(this).attr('stroke', 'black');
    d3.select('#player-name').text(d.Name).style('font-weight', 'bold').style('font-size', 18);
  ...
  d3.select('.btn-centers').on('click', function () {
    console.log('clicked'); // first, make ALL rects grey

    svg.selectAll('.C').attr('fill', 'cyan');
  });
  d3.select('.btn-pointg').on('click', function () {
    console.log('clicked'); // then make lowgdp rects blue
  ...
  d3.select('.btn-reset').on('click', function () {
    console.log('clicked'); // make ALL circles un-stroked
...
},{"d3":"../node_modules/d3/index.js","../data/player_stats.csv":"data/player_stats.csv"}]},{},["scripts/chart-nba.js"], null)
//# sourceMappingURL=/chart-nba.570655bd.js.map