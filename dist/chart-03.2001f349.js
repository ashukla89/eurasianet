...
var svg = d3.select('#chart-03').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // Create your scales

var xPositionScale = d3.scaleLinear().range([0, width]);
var yPositionScale = d3.scaleLinear().range([height, 0]);
var colorScale = d3.scaleOrdinal().range(['#1b9e77', '#d95f02', '#7570b3']); // Do you need a d3.line function for this? Maybe something similar?
...
function ready(datapoints) {
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('stroke', 'black').attr('stroke-width', 1).attr('fill', function (d) {
    // console.log('this nested thing is', d)
    return colorScale(d.key);
  }).attr('d', function (d) {
    return area(d.values);
  }).attr('opacity', 0.5); // Add your axes
...
//     .call(yAxis)
// }
},{"d3":"../node_modules/d3/index.js","../data/air-emissions.csv":"data/air-emissions.csv"}]},{},["scripts/chart-03.js"], null)
