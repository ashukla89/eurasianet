...
console.log('Building chart rec act');
var container = d3.select('#chart-rec-act'); // Create your scales

var xPositionScale = d3.scaleLinear().range([0, width]);
var yPositionScale = d3.scaleLinear().range([height, 0]); // Create a time parser

var parseTime = d3.timeParse('%m-%d-%Y'); // Create your line generator
...
d3.csv(require('../data/all_05-28-2020.csv')).then(ready).catch(function (err) {
  console.log(err);
...
function ready(datapoints) {
  ...
  var cases = datapoints.map(function (d) {
    return +d.Confirmed;
  });
  var minDate = d3.min(dates);
  var maxDate = d3.max(dates);
  var maxCases = d3.max(cases);
  xPositionScale.domain([minDate, maxDate]);
  yPositionScale.domain([0, maxCases]); // Group your data together
...
},{"d3":"../node_modules/d3/index.js","../data/all_05-28-2020.csv":"data/all_05-28-2020.csv"}]},{},["scripts/chart-rec-act.js"], null)
