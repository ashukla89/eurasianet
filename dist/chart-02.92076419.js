...
var colorScale = d3.scaleOrdinal() // .domain(['Australia', 'Korea', 'Estonia', 'Indonesia', 'Colombia'])
.range(['#7fc97f', '#beaed4', '#fdc086', '#ffff99', '#386cb0']); // Create a d3.line function
...
d3.csv(require('../data/alcohol-consumption.csv')).then(ready).catch(function (err) {
  console.log(err);
});
...
function ready(datapoints) {
  var nested = d3.nest().key(function (d) {
    return d.LOCATION;
  }).entries(datapoints); // console.log(nested)
  ...
  var maxVal = d3.max(datapoints, function (d) {
    return +d.Value;
  });
  xPositionScale.domain([minYear, maxYear]);
  yPositionScale.domain([0, maxVal]); // Draw your dots
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('stroke', function (d) {
    // console.log('next', d)
    return colorScale(d.key);
  }).attr('stroke-width', 2).attr('fill', 'none').attr('d', function (d) {
...
},{"d3":"../node_modules/d3/index.js","../data/alcohol-consumption.csv":"data/alcohol-consumption.csv"}]},{},["scripts/chart-02.js"], null)
